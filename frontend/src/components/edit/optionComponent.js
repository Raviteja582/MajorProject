...
class Options extends Component {
  ...
  handleSubmit1(e) {
    ...
    for (var i = 0; i < this.state.questions.length; i++) {
      if (!this.state.removedQuestions[i]) {
        var sp = {
          ...
          name: this.state.questions[i].name.trim(),
        };
        xs.push(sp);
      }
    }
    ...
    editQuestions(
      ...
      this.state.selectedDiffculty.value,
