...
class Schema extends Component {
  ...
  handleSubmit2() {
    ...
    if (
      this.state.mids.date === "" ||
      this.state.mids.month === "" ||
      this.state.mids.start === "" ||
      this.state.mids.end === ""
    )
      ...
      var dat =
        xs.date.getDate() < 10 ? "0" + xs.date.getDate() : xs.date.getDate();
      var mon =
        xs.date.getMonth() < 10 ? "0" + xs.date.getMonth() : xs.date.getMonth();
      xs.date = dat + "/" + mon + "/" + xs.date.getFullYear();

      var sm = xs.start.split(":");
      var mer1 = Number(sm[0]) >= 12 ? "PM" : "AM";
      var hour1 =
        Number(sm[0]) >= 12 ? Number(sm[0]) % 12 || 12 : Number(sm[0]);
      ...
      if (this.state.examtype === "mid1") this.getMid1(xs);
      else this.getMid2(xs);
    }
  }
  ...
  getMid1 = async (details) => {
    ...
    instance
      .post("/teacher/mid1", details)
      .then((response) => {
        const pdfBlob = new Blob([response.data], { type: "application/pdf" });
        let date_ob = new Date();
        var str =
          details.value + "_" + date_ob.getHours() + "_" + date_ob.getMinutes();
  ...
  render() {
    if (this.state.isloading) {
      ...
      if (this.state.selectedSubject === "" || this.state.selectedType === "")
        ...
        this.state.selectedType.value === "sem" &&
        ...
        this.state.selectedType.value === "custom" &&
        this.state.selectedSubject !== ""
      ) {
        value = (
          <Custome
          ...
          <div style={{width: "70%" }}>
            <Form onSubmit={() => this.handleSubmit2()}>
              <Row md={12}>
                <Col md={12} lg={6}>
                  <FormGroup>
                    <Label for="exampleEmail">
                      Select Month and Year of Semester
                    </Label>
                    <br />
                    <DatePicker
                      required
                ...
                <Col md={12} lg={6}>
                  <FormGroup>
                    <Label for="exampleAddress">Date of Exam</Label>
                    <br />
                    <DatePicker
                      required
      ...
      return (
        <div style={{ width: "80%", margin: "3%" }}>
          ...
          <Select
            name="exam"
            options={this.state.examOptions}
